"""
evaluate_ch4_cgan.py — Comprehensive CGAN Evaluation Suite for mine_part2_ch4_realistic.csv
...
Outputs:
  - gas_sensors/evaluation_plots/part2_ch4_*.png
  - docs/PART2_CH4_CGAN_SYNTHETIC_EVALUATION.md
"""
import os
import sys
import warnings
...
REAL_PATH = os.path.join(DATA_DIR, "mine_part2_ch4_realistic.csv")
BALANCED_PATH = os.path.join(DATA_DIR, "mine_part2_ch4_balanced_cgan.csv")
...
def run_evaluation():
    print("=" * 70)
    print("  COMPREHENSIVE CGAN EVALUATION FOR MINE_PART2 CH4 (over_tlv & severity)")
    print("=" * 70)
    ...
    for i, col in enumerate(FEATURE_COLS):
        ax = axes[i]
        sns.histplot(df_real[col], ax=ax, color="#1f77b4", label="Real CH4 Data", kde=True, stat="density", alpha=0.4, bins=30)
        sns.histplot(df_syn[col], ax=ax, color="#ff7f0e", label="Synthetic CGAN CH4", kde=True, stat="density", alpha=0.4, bins=30)
        ax.set_title(f"Feature: {col}", fontsize=11, fontweight="bold")
        ax.set_xlabel("")
        ax.legend(fontsize=9)
    plt.suptitle("Distribution Overlap (Histograms + KDE): Real vs CGAN Synthetic CH4", fontsize=13, fontweight="bold")
    plt.tight_layout()
    plot_hist_path = os.path.join(PLOT_DIR, "part2_ch4_hist_kde.png")
    ...
    plt.suptitle("Side-by-Side Boxplots: Real CH4 vs Synthetic CGAN CH4", fontsize=13, fontweight="bold")
    plt.tight_layout()
    plot_box_path = os.path.join(PLOT_DIR, "part2_ch4_boxplots.png")
    ...
    axes[0].set_title("Real CH4 Correlation Matrix", fontweight="bold")
    ...
    plt.tight_layout()
    plot_corr_path = os.path.join(PLOT_DIR, "part2_ch4_correlation.png")
    ...
    plt.scatter(pca_real[:, 0], pca_real[:, 1], c="#1f77b4", alpha=0.7, label="Real CH4", s=25, edgecolors="k", linewidth=0.3)
    ...
    plt.legend()
    plt.tight_layout()
    plot_pca_path = os.path.join(PLOT_DIR, "part2_ch4_pca.png")
    ...
    plt.scatter(tsne_real[:, 0], tsne_real[:, 1], c="#1f77b4", alpha=0.7, label="Real CH4", s=25, edgecolors="k", linewidth=0.3)
    plt.title("t-SNE 2D Manifold Embedding: Real vs Synthetic CH4", fontsize=12, fontweight="bold")
    plt.xlabel("t-SNE Dimension 1")
    plt.ylabel("t-SNE Dimension 2")
    plt.legend()
    plt.tight_layout()
    plot_tsne_path = os.path.join(PLOT_DIR, "part2_ch4_tsne.png")
    ...
    for idx, (f1, f2) in enumerate(pairs):
        ...
        ax.scatter(df_real[f1], df_real[f2], c="#1f77b4", alpha=0.6, label="Real CH4 Data", s=20, edgecolors="k", linewidth=0.2)
        ax.set_title(f"{f1} vs {f2}", fontweight="bold")
        ax.set_xlabel(f1)
        ax.set_ylabel(f2)
        ax.legend()
    plt.suptitle("Pairwise Feature Scatter Comparisons: Real vs Synthetic CH4", fontsize=13, fontweight="bold")
    plt.tight_layout()
    plot_pair_path = os.path.join(PLOT_DIR, "part2_ch4_pairwise_scatter.png")
...
def generate_markdown_report(res, df_real, df_balanced):
    report_path = os.path.join(DOCS_DIR, "PART2_CH4_CGAN_SYNTHETIC_EVALUATION.md")

    md = []
    md.append("# PyTorch CGAN Synthetic Data Evaluation Report: Methane (CH4)")
    md.append("## Dataset: `mine_part2_ch4_realistic.csv` | Target Features: `over_tlv` & `severity`\n")
    md.append("This report provides a comprehensive 13-parameter evaluation of the PyTorch Conditional GAN synthesized dataset (`mine_part2_ch4_balanced_cgan.csv`) compared against ground-truth real CH4 telemetry data.\n")
    ...
    md.append(f"- **Target Constants Enforced**: `tlv_pct = 2.5` | `tlv_ppm = 25000.0` (100% constant across all rows)")
    md.append(f"- **`over_tlv` Index Threshold**: 0 for `< 2.5%` / `< 25,000 ppm`, 1 for `>= 2.5%` / `>= 25,000 ppm`\n")
    ...
    md.append("### Real CH4 Data vs Synthetic CGAN CH4 Data (Mean & Std)\n")
    ...
    md.append("- **Histograms + KDE Overlap**: `gas_sensors/evaluation_plots/part2_ch4_hist_kde.png`")
    md.append("- **Boxplots Comparison**: `gas_sensors/evaluation_plots/part2_ch4_boxplots.png`")
    md.append("- **Correlation Heatmaps**: `gas_sensors/evaluation_plots/part2_ch4_correlation.png`")
    md.append("- **PCA 2D Projection**: `gas_sensors/evaluation_plots/part2_ch4_pca.png`")
    md.append("- **t-SNE 2D Manifold**: `gas_sensors/evaluation_plots/part2_ch4_tsne.png`")
    md.append("- **Pairwise Scatter Plots**: `gas_sensors/evaluation_plots/part2_ch4_pairwise_scatter.png`")
