"""
gan_generate_h2.py — PyTorch CGAN for Hydrogen (H2)
...
Performs TWO separate CGAN balancing passes:
  ...
  Pass 2: Balance 'over_tlv' (0,1) -> N_max = 16,272
          Class 1: 16,272 samples (majority)
          Class 0: 13,728 samples -> synthesize 2,544 CGAN samples

Output: gas_sensors/data/mine_part2_h2_balanced_cgan.csv
"""
import os
import pandas as pd
import numpy as np
...
def run():
    print("=" * 60)
    print("PYTORCH CGAN: H2 — severity + over_tlv BALANCING")
    print("=" * 60)
    ...
    for i, orig_cls in inv_map.items():
        ...
        if needed > 0:
            ...
            # Severity mapping for H2: L1=0 (<= 2.0% TLV), L2=1 (mid), L3=2
