"""
gan_generate_h2.py — PyTorch CGAN for Hydrogen (H2) Dataset Balancing
...
Computes over_tlv deterministically from physical thresholding (pct >= 2.0% / ppm >= 20000.0).
...
def run():
    ...
    for sev in [0, 1, 2]:
        ...
        if needed > 0:
            ...
            if sev == 0:
                df_gen["pct"] = np.clip(df_gen["pct"], 0.0, 1.80)
            elif sev == 1:
                df_gen["pct"] = np.clip(df_gen["pct"], 1.80, 2.50)
            else:
                df_gen["pct"] = np.clip(df_gen["pct"], 2.50, 3.80)
