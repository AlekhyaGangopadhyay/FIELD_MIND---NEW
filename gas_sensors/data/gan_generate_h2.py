"""
gan_generate_h2.py — PyTorch CGAN for Hydrogen (H2) Balancing over_tlv & severity

Performs CGAN dataset balancing across target features: 'over_tlv' (0,1) and 'severity' (0,1,2).
Joint classes (severity, over_tlv):
  (0, 0): Real 553 samples -> Synthesize 9,447 CGAN samples
  (0, 1): Real 9,447 samples -> Synthesize 553 CGAN samples
...
Output file: gas_sensors/data/mine_part2_h2_balanced_cgan.csv
"""
import os
import numpy as np
import pandas as pd
...
# Constant properties for H2 TLV
TLV_PCT_CONST = 0.02
TLV_PPM_CONST = 200.0
...
def run():
    print("=" * 70)
    print("  PYTORCH CGAN: H2 SYNTHESIS FOR over_tlv & severity BALANCING")
    print("=" * 70)
    ...
    seed_dfs.append(pd.DataFrame({
        "gas": "H2", "level": "L2", "pct": pct_c1, "ppm": ppm_c1,
    ...
    seed_dfs.append(pd.DataFrame({
        "gas": "H2", "level": "L3", "pct": pct_c2, "ppm": ppm_c2,
        "tlv_pct": TLV_PCT_CONST, "tlv_ppm": TLV_PPM_CONST,
