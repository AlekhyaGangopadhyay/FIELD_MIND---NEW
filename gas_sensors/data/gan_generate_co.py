"""
gan_generate_co.py — PyTorch CGAN for Carbon Monoxide (CO) Dataset Balancing
...
Computes over_tlv deterministically from physical thresholding (pct >= 0.005% / ppm >= 50.0).
...
def run():
    print("=" * 70)
    print("  PYTORCH CGAN: CARBON MONOXIDE (CO) PHYSICAL SEVERITY BALANCING")
    print("=" * 70)
    ...
    if os.path.exists(INPUT_CSV):
        ...
        df_real = df_all[df_all["gas"] == "CO"].copy()
        if "ppm_noisy" not in df_real.columns:
            df_real["ppm_noisy"] = df_real["ppm"] + np.random.normal(0, 5, len(df_real))
    ...
    for sev in [0, 1, 2]:
        ...
        if needed > 0:
            ...
            if sev == 0:
                df_gen["pct"] = np.clip(df_gen["pct"], 0.0, 0.00375)
            elif sev == 1:
                df_gen["pct"] = np.clip(df_gen["pct"], 0.00375, 0.0050)
            else:
                df_gen["pct"] = np.clip(df_gen["pct"], 0.0050, 1.000)

            df_gen["ppm"] = df_gen["pct"] * 10000.0
            noise_std = np.std(df_real["ppm_noisy"] - df_real["ppm"])
            df_gen["ppm_noisy"] = np.clip(df_gen["ppm"] + np.random.normal(0, max(noise_std, 0.5), len(df_gen)), 0, None)
