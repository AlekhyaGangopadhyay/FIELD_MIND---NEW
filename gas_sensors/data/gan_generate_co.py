"""
gan_generate_co.py — PyTorch CGAN for Carbon Monoxide (CO) Balancing over_tlv & severity
...
Joint classes (severity, over_tlv):
  ...
  (2, 0): Real 4 samples -> Synthesize 9,996 CGAN samples
  (0, 1): Synthesize 10,000 CGAN samples
  (1, 1): Synthesize 10,000 CGAN samples
  (2, 1): Real 9,996 samples -> Synthesize 4 CGAN samples

Total output dataset: 60,000 rows (perfectly balanced: 10,000 per joint class).
Output file: gas_sensors/data/mine_part2_co_balanced_cgan.csv
"""
import os
import numpy as np
import pandas as pd
...
# Constant properties for CO TLV
TLV_PCT_CONST = 0.005
TLV_PPM_CONST = 50.0
...
def run():
    print("=" * 70)
    print("  PYTORCH CGAN: CO SYNTHESIS FOR over_tlv & severity BALANCING")
    print("=" * 70)
    ...
    # Verify over_tlv in real data (0 if pct < 0.005 else 1)
    ...
    # Construct representative seeds for missing joint classes (0,1) and (1,1) so CGAN can learn all conditions
    seed_dfs = [df_real.copy()]
    noise_std = np.std(df_real["ppm_noisy"] - df_real["ppm"])

    # Seed for Class 3 (severity 0, over_tlv 1): pct in [0.0050, 0.0070]
    n_seed = 2000
    pct_c3 = np.random.uniform(0.0050, 0.0070, n_seed)
    ppm_c3 = pct_c3 * 10000.0
    seed_dfs.append(pd.DataFrame({
        "gas": "CO", "level": "L1", "pct": pct_c3, "ppm": ppm_c3,
        "tlv_pct": TLV_PCT_CONST, "tlv_ppm": TLV_PPM_CONST,
        "over_tlv": 1, "severity": 0, "ppm_noisy": ppm_c3 + np.random.normal(0, noise_std, n_seed),
        "joint_cls": 3
    }))

    # Seed for Class 4 (severity 1, over_tlv 1): pct in [0.0070, 0.0090]
    pct_c4 = np.random.uniform(0.0070, 0.0090, n_seed)
    ppm_c4 = pct_c4 * 10000.0
    seed_dfs.append(pd.DataFrame({
        "gas": "CO", "level": "L2", "pct": pct_c4, "ppm": ppm_c4,
        "tlv_pct": TLV_PCT_CONST, "tlv_ppm": TLV_PPM_CONST,
        "over_tlv": 1, "severity": 1, "ppm_noisy": ppm_c4 + np.random.normal(0, noise_std, n_seed),
        "joint_cls": 4
    }))
    ...
    for jc in range(6):
        ...
        if needed > 0:
            ...
            if otlv == 1:
                # Ensure physical consistency for over_tlv = 1 (pct >= 0.005 / ppm >= 50.0)
                df_gen["pct"] = np.clip(df_gen["pct"], 0.0050, 0.0250)
                df_gen["ppm"] = df_gen["pct"] * 10000.0
                df_gen["ppm_noisy"] = np.clip(df_gen["ppm_noisy"], df_gen["ppm"] - 15, df_gen["ppm"] + 15)
                df_gen["over_tlv"] = 1
            else:
                # Ensure physical consistency for over_tlv = 0 (pct < 0.005 / ppm < 50.0)
                df_gen["pct"] = np.clip(df_gen["pct"], 0.0001, 0.00499)
                df_gen["ppm"] = df_gen["pct"] * 10000.0
