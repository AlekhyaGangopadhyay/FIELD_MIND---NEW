"""
gan_generate_ch4.py — PyTorch CGAN for Methane (CH4) Balancing over_tlv & severity

Performs CGAN dataset balancing across target features: 'over_tlv' (0,1) and 'severity' (0,1,2).
Joint classes (severity, over_tlv):
  (0, 0): Real 10,000 samples
  (1, 0): Real 10,000 samples
  (2, 0): Real 10,000 samples
  (0, 1): Synthesize 10,000 CGAN samples (pct >= 2.5% / ppm >= 25,000)
  (1, 1): Synthesize 10,000 CGAN samples (pct >= 2.5% / ppm >= 25,000)
  (2, 1): Synthesize 10,000 CGAN samples (pct >= 2.5% / ppm >= 25,000)
...
# Constant properties for CH4 TLV
TLV_PCT_CONST = 2.5
...
def train_cgan(X_scaled, y_joint, num_classes=6, noise_dim=32, epochs=50, batch_size=256):
...
def run():
    print("=" * 70)
    print("  PYTORCH CGAN: CH4 SYNTHESIS FOR over_tlv & severity BALANCING")
    print("=" * 70)

    df_real = pd.read_csv(INPUT_CSV)
    print(f"Loaded {len(df_real):,} rows from {INPUT_CSV}")

    # Enforce constant tlv_pct and tlv_ppm in real data
    df_real["tlv_pct"] = TLV_PCT_CONST
    df_real["tlv_ppm"] = TLV_PPM_CONST

    # Verify over_tlv in real data (0 if pct < 2.5 else 1)
    ...
    # Prepare training dataset for CGAN
    # To enable CGAN to generate over_tlv = 1 (pct >= 2.5%), we construct representative seed data for over_tlv=1 classes
    # based on physical CH4 telemetry relationships and real noise distribution.
    seed_dfs = [df_real.copy()]

    for sev in [0, 1, 2]:
        # Generate domain-informed base seeds for over_tlv = 1 (pct >= 2.5%)
        # Sev 0: pct in [2.5, 3.2], Sev 1: pct in [3.2, 4.0], Sev 2: pct in [4.0, 5.0]
        n_seed = 2000
        if sev == 0:
            pct_vals = np.random.uniform(2.50, 3.20, n_seed)
        elif sev == 1:
            pct_vals = np.random.uniform(3.20, 4.00, n_seed)
        else:
            pct_vals = np.random.uniform(4.00, 5.00, n_seed)

        ppm_vals = pct_vals * 10000.0
        noise_std = np.std(df_real["ppm_noisy"] - df_real["ppm"])
        ppm_noisy_vals = ppm_vals + np.random.normal(0, noise_std, n_seed)
        ...
        df_seed = pd.DataFrame({
            ...
            "pct": pct_vals,
            "ppm": ppm_vals,
            "tlv_pct": TLV_PCT_CONST,
            "tlv_ppm": TLV_PPM_CONST,
            "over_tlv": 1,
            "severity": sev,
            "ppm_noisy": ppm_noisy_vals,
            "joint_cls": sev + 3
        })
    ...
    for jc in range(6):
        ...
        if needed > 0:
            ...
            if otlv == 1:
                # Ensure physical consistency for over_tlv = 1 (pct >= 2.5 / ppm >= 25000)
                df_gen["pct"] = np.clip(df_gen["pct"], 2.50, 6.00)
                df_gen["ppm"] = df_gen["pct"] * 10000.0
                df_gen["ppm_noisy"] = np.clip(df_gen["ppm_noisy"], df_gen["ppm"] - 1000, df_gen["ppm"] + 1000)
                df_gen["over_tlv"] = 1
            else:
                df_gen["pct"] = np.clip(df_gen["pct"], 0.0, 2.49)
                df_gen["ppm"] = df_gen["pct"] * 10000.0
