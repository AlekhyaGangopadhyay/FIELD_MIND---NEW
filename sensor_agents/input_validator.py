...
VALID_RANGES = {
    ...
    "MQ135_NH3_ppm": {"min": 0.0, "max": 500.0, "dead_thresh": 0.1},
    "MQ136_H2S_ppm": {"min": 0.0, "max": 500.0, "dead_thresh": 0.01},
...
def validate_sensor(sensor_name: str, value: float, history: List[float]) -> SensorHealthReport:
    ...
    # Dead detection (first, because if it's dead, range validation might clamp or fail it)
    if detect_dead_sensor(sensor_name, value):
        report.is_dead = True
        report.status = "DEAD"
        report.is_valid = False
        report.clamped_value = value
        return report
